function swiper() {
  ...
  const load=()=>{
      setTimeout(() => {
          txt.textContent="traditional remedies."
      }, 0);
      setTimeout(() => {
          txt.textContent="community-driven."
      }, 2000);
      setTimeout(() => {
          txt.textContent="rural health."
      }, 4000);
      setTimeout(() => {
          txt.textContent="herbal remedies."
      }, 6000);
      setTimeout(() => {
          txt.textContent="homegrown solutions."
      }, 8000);
      setTimeout(() => {
          txt.textContent="knowledge sharing."
      }, 10000);
      setTimeout(() => {
          txt.textContent="collaboration."
      }, 12000);
      setTimeout(() => {
          txt.textContent="empowering women."
      }, 14000);
      setTimeout(() => {
          txt.textContent="wellness exchange."
      }, 16000);
  }
  load();
  setInterval(load,18000)
  var h1Text = document.querySelector("#page-content p").textContent;
 function   spanAnimation (){
    var splittedText = h1Text.split("");
  var clutter = "";
  splittedText.forEach(function(elem) {
      clutter += `<span>${elem}</span>`; // Corrected backticks usage
  });
  
  document.querySelector("#page-content p").innerHTML = clutter;
  
  gsap.to("#page-content p span", {
      color: "rgb(147, 106, 106)",
      stagger: 0.1,
      scrollTrigger: {
